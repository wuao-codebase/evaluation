...
public class MinZuxue {
    public static void main(String[] args) {
        ...
        try{
            ...
            List<String> list = Arrays.asList("30401","030303T");
            List<Evaluat> eval = evalMapper.getEval(list);
            for (Evaluat evaluat : eval) {
                StringBuilder evals = new StringBuilder();
                evals.append(evalService.compnear("总学分", evaluat.getBYXFZS(), 163));
                ...
                evals.append(evalService.comptowmin("生均年教学运行支出(万元)",evaluat.getDNJXJF() ,evaluat.getZXSRS(), 0.3));
                if ((evals).length()>0) {
            ...
            FileWriter writer = new FileWriter(args[0]+"\\民族学.txt");
            writer.write( "————民族学————\r\n");
